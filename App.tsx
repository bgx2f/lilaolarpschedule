        ...
        <div className="w-full flex flex-col md:flex-row justify-between items-center gap-3">
          <div className="flex items-center gap-4 w-full md:w-auto justify-between">
             ...
             <div className="flex items-center gap-2">
                <div className="flex items-center bg-gray-100 p-1 rounded-xl border border-gray-200">
                   ...
                   <button onClick={() => setZoomLevel(1.0)} className="px-3 text-xs font-black text-gray-500 hover:text-gray-900 transition-colors" title="重設縮放">
                ...
                <button onClick={() => setIsSearchModalOpen(true)} className="p-3 bg-gray-100 hover:bg-gray-200 rounded-xl text-gray-700 transition-all active:scale-90 border border-gray-200 shadow-sm" title="全域搜尋">
                    <Search size={20} />
                </button>
                
                {canEdit && (
                    <div className="bg-gray-100 p-1.5 rounded-2xl flex border border-gray-200 shadow-sm">
                        <button onClick={() => setViewMode('public')} className={`px-4 py-2 rounded-xl text-xs font-black flex items-center gap-2 transition-all ${viewMode === 'public' ? 'bg-white text-teal-600 shadow-md' : 'text-gray-500 hover:text-gray-700'}`}>
                            <LayoutGrid size={16} /> 班表
                        </button>
                        <button onClick={() => setViewMode('admin')} className={`px-4 py-2 rounded-xl text-xs font-black flex items-center gap-2 transition-all ${viewMode === 'admin' ? 'bg-white text-larp-primary shadow-md' : 'text-gray-500 hover:text-gray-700'}`}>
                            <List size={16} /> 管理
                        </button>
                    </div>
                )}
          ...
          {currentUser && viewMode === 'admin' && (
            <div className="flex items-center justify-center gap-3 animate-in fade-in duration-300">
                <button onClick={() => setSelectedDate(new Date(new Date(selectedDate).setDate(new Date(selectedDate).getDate() - 1)).toISOString().split('T')[0])} className="p-3 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl shadow-sm transition-all active:scale-90"><ChevronLeft size={20}/></button>
                <div className="px-8 py-2.5 bg-white rounded-2xl shadow-md border border-gray-200 text-center min-w-[180px]">
                    <span className="text-xl font-black text-gray-900 font-mono tracking-wider">{selectedDate}</span>
                </div>
                <button onClick={() => setSelectedDate(new Date(new Date(selectedDate).setDate(new Date(selectedDate).getDate() + 1)).toISOString().split('T')[0])} className="p-3 bg-white border border-gray-200 hover:bg-gray-50 rounded-xl shadow-sm transition-all active:scale-90"><ChevronRight size={20}/></button>
            </div>
          )}
          ...
          <div className="flex items-center gap-4">
              ...
              <div className="relative">
                  ...
                  >
                    ...
                    <span className="text-xs font-black">{onlineMembers.length} 人在線</span>
                  </button>

                  {/* Online Users Tooltip List */}
              ...
              {currentUser && (
                  <div className="flex items-center gap-3">
                    <div className="hidden sm:flex items-center gap-3 px-4 py-2 bg-gray-900 rounded-2xl border border-gray-800 shadow-lg">
                        <UserIcon size={16} className="text-teal-400"/>
                        <span className="text-sm font-black text-white">{currentUser.displayName}</span>
                    </div>
                    <button onClick={handleLogout} className="p-3 text-gray-500 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all" title="登出系統"><LogOut size={24} /></button>
                  </div>
              )}
          </div>
        </div>
        ...
        {!isDataLoaded ? (
            ...
            <div className="flex-1 flex flex-col w-full h-full pb-12 overflow-hidden">
                {viewMode === 'admin' && canEdit && (
                    <div className="h-full flex flex-col animate-in fade-in duration-300 p-4 w-full">
                        <div className="mb-4 flex-shrink-0 flex items-center gap-4 sticky left-0 z-[60]">
                             <button onClick={() => setIsStatsModalOpen(true)} className="flex items-center gap-2 px-6 py-3 border border-indigo-200 rounded-2xl text-sm font-black shadow-sm text-indigo-700 bg-white hover:bg-indigo-50 transition-all"><BarChart3 size={20} /> 統計報表</button>
                             <button onClick={() => { setListForcePending(false); setIsListModalOpen(true); }} className="flex items-center gap-2 px-6 py-3 border border-gray-300 rounded-2xl text-sm font-black shadow-sm text-gray-700 bg-white hover:bg-gray-50 transition-all"><Table2 size={20} /> 預約清單</button>
                             
                             <button 
                                onClick={() => { setListForcePending(true); setIsListModalOpen(true); }} 
                                className={`flex items-center gap-2 px-6 py-3 rounded-2xl text-sm font-black shadow-sm transition-all border ${pendingStaffCount > 0 ? 'bg-orange-50 border-orange-200 text-orange-700 hover:bg-orange-100' : 'bg-gray-50 border-gray-200 text-gray-400 opacity-60 cursor-default'}`}
                                disabled={pendingStaffCount === 0}
                             >
                                <AlertTriangle size={20} className={pendingStaffCount > 0 ? 'text-orange-500' : ''} /> 
                                待定場次 
                                {pendingStaffCount > 0 && <span className="ml-1 bg-orange-500 text-white px-2 py-0.5 rounded-full text-[10px] shadow-sm">{pendingStaffCount}</span>}
                             </button>

                             <button onClick={() => setIsSettingsModalOpen(true)} className="flex items-center gap-2 px-6 py-3 border border-gray-300 rounded-2xl text-sm font-black shadow-sm text-gray-700 bg-white hover:bg-gray-50 transition-all ml-auto"><Settings size={20} /> 系統設定</button>
                        </div>
                        
                        <div className="flex-1 overflow-auto border border-gray-200 rounded-3xl bg-white/40 shadow-inner no-scrollbar h-full">
                            <div 
                                className="w-full p-6 min-h-full" 
