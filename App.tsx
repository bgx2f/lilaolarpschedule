...
const App: React.FC = () => {
  ...
  const handleUpdateAdminPassword = (newPassword: string) => {
      ...
      setMembers(prev => prev.map(m => m.id === currentUser.id ? { ...m, password: newPassword } : m));
  ...
  // If not authenticated, we only show the main structure blurred or hidden behind the modal.
  // But LoginModal is fixed with z-index 2000, so we can render the app 'behind' it.
  // However, for security, we shouldn't render sensitive data if not logged in.
  ...
  return (
    <div className={`min-h-screen ${backgroundClass} text-gray-900 flex flex-col font-sans transition-colors duration-500`}>
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200 px-4 py-3 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3 md:gap-0">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
             ...
             {/* View Toggle Pill (Only for Admin) */}
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                  ...
                  <>
                    ...
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-100 rounded-lg border border-gray-200">
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-[10px] text-gray-500 leading-none mt-0.5 capitalize">{currentUser.role === MemberRole.ADMIN ? '管理員' : '主持人'}</span>
                        </div>
                    </div>
                    <button 
              ...
              {/* Add Button (Only in Admin) */}
              {viewMode === 'admin' && currentUser?.role === MemberRole.ADMIN && (
                <button 
      ...
      <main className="flex-1 overflow-x-auto overflow-y-hidden p-4 relative">
        ...
        {/* If no user, show placeholder or nothing (Login Modal is overlay) */}
        {!currentUser ? (
            ...
            <>
                {/* VIEW 1: ADMIN DASHBOARD (SWIMLANE) */}
                {viewMode === 'admin' && currentUser.role === MemberRole.ADMIN && (
      ...
      {/* Modal: Booking List View with Global History */}
      <BookingListModal 
      ...
      {currentUser && (
          <SettingsModal 
            ...
            adminPassword={currentUser.password} // Pass current user password for legacy check or verification
          />
      )}
    </div>
  );
