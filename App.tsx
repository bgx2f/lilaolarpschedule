...
import { ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, Calendar, Plus, Filter, Settings, LayoutGrid, List, CalendarSearch, CalendarCheck, CalendarOff, Table2, BarChart3, LogOut, User as UserIcon, Cloud, Loader2, Save } from 'lucide-react';
import { Booking, Room, SlotType, Member, MemberRole, MemberStatus, LarpData, GoogleDriveConfig } from './types';
...
const App: React.FC = () => {
  // Persist selected date for better UX
  ...
  const [driveConfig, setDriveConfig] = useState<GoogleDriveConfig>(() => loadState('larp_drive_config', { clientId: '', apiKey: '' }));
  const [isDriveConnected, setIsDriveConnected] = useState(false);
  const [isCloudModalOpen, setIsCloudModalOpen] = useState(false);
  const [lastSynced, setLastSynced] = useState<string | null>(() => loadState('larp_last_synced', null));
  const [isSyncing, setIsSyncing] = useState(false);
  const fileIdRef = useRef<string | null>(null);

  // Modal States
  const [isLoginModalOpen, setIsLoginModalOpen] = useState(() => !loadState('larp_current_user', null));
  ...
  useEffect(() => localStorage.setItem('larp_bookings', JSON.stringify(bookings)), [bookings]);
  useEffect(() => localStorage.setItem('larp_rooms', JSON.stringify(rooms)), [rooms]);
  useEffect(() => localStorage.setItem('larp_scripts', JSON.stringify(scripts)), [scripts]);
  useEffect(() => localStorage.setItem('larp_dms', JSON.stringify(dms)), [dms]);
  useEffect(() => localStorage.setItem('larp_npcs', JSON.stringify(npcs)), [npcs]);
  useEffect(() => localStorage.setItem('larp_holidays', JSON.stringify(holidays)), [holidays]);
  useEffect(() => localStorage.setItem('larp_members', JSON.stringify(members)), [members]);
  useEffect(() => localStorage.setItem('larp_view_mode', JSON.stringify(viewMode)), [viewMode]);
  useEffect(() => localStorage.setItem('larp_selected_date', JSON.stringify(selectedDate)), [selectedDate]);
  ...
  // Save Drive Config
  useEffect(() => localStorage.setItem('larp_drive_config', JSON.stringify(driveConfig)), [driveConfig]);
  useEffect(() => {
      if(lastSynced) localStorage.setItem('larp_last_synced', lastSynced);
  }, [lastSynced]);

  // --- Cloud Sync Logic ---
  const collectData = (): LarpData => ({
      bookings, rooms, scripts, dms, npcs, holidays, members,
      lastUpdated: new Date().toISOString()
  });

  const applyData = (data: LarpData) => {
      ...
      // If the current user's data exists in the imported member list, update their session
      if (currentUser) {
  ...
  const handleSyncUp = async () => {
      ...
      if (!fileId) {
          fileId = await DriveApi.findDbFile();
      }

      if (fileId) {
          await DriveApi.updateDbFile(fileId, data);
          fileIdRef.current = fileId;
      } else {
          const newId = await DriveApi.createDbFile(data);
  ...
  const handleSyncDown = async () => {
      if (!isDriveConnected) throw new Error("未連接 Google Drive");
      let fileId = fileIdRef.current;
      if (!fileId) {
          fileId = await DriveApi.findDbFile();
          fileIdRef.current = fileId;
      ...
      if (fileId) {
          const data = await DriveApi.readDbFile(fileId);
          applyData(data);
      } else {
          throw new Error("雲端找不到備份檔案 (larp_schedule_db.json)");
      }
  };

  // Auto-save debounce effect (Only if connected and is ADMIN)
  useEffect(() => {
      if (!isDriveConnected || currentUser?.role !== MemberRole.ADMIN) return;
  ...
  }, [bookings, rooms, scripts, dms, npcs, holidays, members, isDriveConnected, currentUser?.role]);
  ...
  return (
    <div className={`min-h-screen ${backgroundClass} text-gray-900 flex flex-col font-sans transition-colors duration-500`}>
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200 px-4 py-3 shadow-sm">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-3 md:gap-0">
          ...
          <div className="flex items-center gap-3 w-full md:w-auto justify-end">
              {currentUser ? (
                  <>
                     {/* Cloud Sync Button - RESTRICTED TO ADMIN ONLY */}
                     {isAdmin && (
                         <button 
                            onClick={() => setIsCloudModalOpen(true)}
                            className={`p-2 rounded-lg transition-colors border relative ${isDriveConnected ? 'bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100' : 'bg-gray-50 text-gray-400 border-gray-200 hover:bg-gray-100'}`}
                            title="雲端資料庫管理 (僅限管理員)"
                         >
      ...
      {/* Cloud Sync Config Modal - Only if ADMIN */}
      {isAdmin && (
          <CloudSyncModal 
