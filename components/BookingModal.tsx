...
import { X, Save, Trash2, AlertCircle, Plus, Check, DollarSign, ShieldCheck, RefreshCw } from 'lucide-react';
      ...
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-2xl border border-gray-100 max-h-[90vh] flex flex-col relative animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex-1 overflow-y-auto p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input list="scripts" type="text" className="w-full bg-gray-50 border border-gray-200 rounded-lg p-2.5 text-gray-900 focus:ring-2 focus:ring-larp-primary outline-none font-bold" value={formData.scriptName} onChange={e => setFormData({...formData, scriptName: e.target.value})} placeholder="輸入或選擇劇本..."/>
                <datalist id="scripts">{availableScripts.map(s => <option key={s} value={s} />)}</datalist>
                </div>
            </div>
            ...
            <div className="flex gap-2 mb-2">
                <input type="text" placeholder="輸入DM名字" className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:border-yellow-500" value={dmInput} onChange={e => setDmInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), handleAddDM())}/>
            ...
            <div className="flex flex-wrap gap-2 min-h-[30px]">
                ...
                <span key={idx} className="bg-yellow-50 text-yellow-700 border border-yellow-200 px-2 py-1 rounded text-sm flex items-center font-medium">
                    {dm}<button type="button" onClick={() => removeDM(idx)} className="ml-2 hover:text-red-500 text-yellow-400"><X size={12}/></button>
                </span>
                ))}
            </div>
            </div>
            ...
            <div className="flex gap-2 mb-2">
                <input type="text" placeholder="輸入NPC名字" className="flex-1 bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 text-sm text-gray-900 focus:outline-none focus:border-gray-500" value={npcInput} onChange={e => setNpcInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && (e.preventDefault(), handleAddNPC())}/>
            ...
            <div className="flex flex-wrap gap-2">
                ...
                <span key={idx} className="bg-gray-100 text-gray-700 border border-gray-300 px-2 py-1 rounded text-sm flex items-center">
                    {npc}<button type="button" onClick={() => removeNPC(idx)} className="ml-2 hover:text-red-500 text-gray-400"><X size={12}/></button>
                </span>
                ))}
            </div>
            </div>
